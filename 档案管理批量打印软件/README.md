...
> 当前项目目录已支持本地虚拟环境 `.venv`，下载后即可直接执行 `run.sh` 或 `run.bat`。
...
```bash
./run.sh --excel data.xlsx --template template.docx --output generated --map 盒号=BOX_NO --map 文号=DOC_NO
```

Windows 上可直接运行：

```bat
run.bat --excel data.xlsx --template template.docx --output generated --map 盒号=BOX_NO --map 文号=DOC_NO
```
